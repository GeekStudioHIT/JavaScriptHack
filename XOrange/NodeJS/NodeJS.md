...
- 异步
	- JS 本身是单线程运行的，不可能在一段代码还未结束执行时去执行别的代码。
	- JS是单线程运行，这决定了JS在执行完一段代码之前无法执行包括回调函数在内的别的代码。也就是说，即使平行线程完成工作了，通知JS主线程执行回调函数了，回调函数也要等到JS主线程空闲时才能开始执行。
	- 代码设计模式
		- 函数返回值
			- 同步 一个函数的输出是一个函数的输入
			- 异步 回调函数传递返回值
		- 遍历数组
			- 异步 循环结束后无法保证所有数组成员都处理完毕。？？？
		- 异常处理
			- try-catch 只能同步
		- domain ???
